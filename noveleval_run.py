...
# changed by mehak
        queries = pd.read_csv(args.run.pyserini_index+'-test', sep='\t', header=None, names=['query_id', 'query'])
        topics = {}
        ...
        docstore = LuceneSearcher(args.run.pyserini_index+'-index') # change by mehak --> the index is stored in a dir
    ...
    with open(args.run.run_path, 'r') as f:
        ...
        for line in tqdm(f):
            ...
            if args.run.ir_dataset_name is not None:
                ...
                # text = data['text']
                text = data['contents'] # changed by mehak
                if 'title' in data:
