...
def create_permutation_instruction_chat(
    query: str, docs: List[SearchResult], model_name="gpt-3.5-turbo"
):
    num = len(docs)
...
class ListwiseLlmRanker(OpenAiListwiseLlmRanker):
    ...
    def __init__(
        ...
        if self.config.model_type == "t5":
            ...
            self.decoder_input_ids = (
                self.tokenizer.encode(
                    f"{self.tokenizer.pad_token} Passage",
                    return_tensors="pt",
                    add_special_tokens=False,
                ).to(self.device)
                if self.tokenizer
            ...
            self.target_token_ids = self.tokenizer.batch_encode_plus(
                [
                    f"{self.tokenizer.pad_token} Passage {self.CHARACTERS[i]}"
                    for i in range(len(self.CHARACTERS))
                ],
                return_tensors="pt",
                add_special_tokens=False,
    ...
    def compare(self, query: str, docs: List):
        ...
        if self.scoring == "generation":
            if self.config.model_type == "t5":
                ...
                output_ids = self.llm.generate(input_ids, max_new_tokens=50)[0]
                self.total_completion_tokens += output_ids.shape[0]
                output = self.tokenizer.decode(
            ...
            with torch.no_grad():
                ...
                ranked = sorted(
                    zip([f"[{str(i+1)}]" for i in range(len(docs))], scores),
                    key=lambda x: x[1],
                    reverse=True,
                )
