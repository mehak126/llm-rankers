...
Output Passage A or Passage B:"""
    ...
    def compare(self, query: str, docs: List):
        ...
        if self.config.model_type == "t5":
            ...
            output_ids = self.llm.generate(
                input_ids, decoder_input_ids=self.decoder_input_ids, max_new_tokens=2
            )
            ...
            input_ids = self.tokenizer(
                [prompt0, prompt1], return_tensors="pt"
    ...
    def rerank(self, query: str, ranking: List[SearchResult]) -> List[SearchResult]:
        ...
        if self.method == "allpair":
            ...
            for batch_inputs in tqdm(loader):
                ...
                batch_outputs = self.llm.generate(
                    ...
                    decoder_input_ids=(
                        ...
                        if self.decoder_input_ids.shape[0]
                ...
                self.total_completion_tokens += (
                    batch_outputs.shape[0] * batch_outputs.shape[1]
                )
            ...
            ranking = sorted(
                [
                    SearchResult(docid=docid, score=score, text=None)
            ...
            ranking = [
                SearchResult(docid=doc.docid, score=-i, text=None)
...
class DuoT5LlmRanker(PairwiseLlmRanker):
    def compare(self, query: str, docs: List[str]) -> bool:
        ...
        inputs = [
            self.prompt.format(query=query, doc1=docs[0], doc2=docs[1]),
...
class OpenAiPairwiseLlmRanker(PairwiseLlmRanker):
    def __init__(
        self, model_name_or_path, api_key, method="heapsort", batch_size=2, k=10
    ):
...
Output Passage A or Passage B:"""
    ...
    def compare(self, query: str, docs: List):
        ...
        return [
            f"Passage {self._get_response(input_texts[0])}",
            f"Passage {self._get_response(input_texts[1])}",
        ]
